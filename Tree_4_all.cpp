...
int GetChildHeight(TreeNode * node)
{
    if (node == NULL)
    {
        return 0;
    ...
    int L = GetChildHeight(node->L);
    int R = GetChildHeight(node->R);
    if (L == -1 || R == -1|| (L-R)>1||(R-L)>1)
    {
        return -1;
    }
    int MaxChildLength = L > R ? L:R;
    return MaxChildLength + 1;
}
int GetSelectChildHeight(TreeNode * node, TreeNode *SelectNode,int hight)
{
    if (node == NULL)
    {
        return -1;
    ...
    int L=GetSelectChildHeight(node->L, SelectNode, hight+1);
    int R=GetSelectChildHeight(node->R, SelectNode, hight+1);
    if (L != -1 )
    {
        return L;
    }
...
int GetSelectChildHeightIteration(TreeNode * node, TreeNode *SelectNode)
{
    int height = 1;
    while (1)
    {
...
void PrintNumAbrove(TreeNode *node,int num)
{
    while (1)
    {
        if (node == NULL){
    ...
    //std::cout << "while (1) PrintNode:" << node->data << "\n";
    TreeNode *buff= node;
    std::stack<TreeNode *> sLib;
    std::stack<TreeNode *> s;
    node = node->R;
    while (s.size()>0||node!=NULL)
    {
...
int main()
{
    ...
    ElemType data[size] = { 7,4,10,2,6,8,11,1,3,5 };
    ElemType data2[size2] = { 5,6,7,8, 9, 6, 7, 8, 12,3  };
    TreeNode *nodeAdd= new TreeNode[1];
    memset(nodeAdd, 0, sizeof(TreeNode));
    nodeAdd->data = 17;
    nodeAdd->R = NULL;
    nodeAdd->L = NULL;
    ...
    //_root->L->L->L->L = nodeAdd;
    //std::cout<<"GetChildHeight:"<< GetSelectChildHeightIteration(_root, _root->L->L->R)<<"\n";
    PrintNumAbrove(_root,9);
    //SwapChild(_root);
    //Travel_M(_root);
    int value = 0;
